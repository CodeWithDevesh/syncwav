...
namespace swav {
	namespace log {
		...
		inline void SWAV_API d(fmt::format_string<Args...> fmt, Args&&... args) {
			Logger::get()->debug(fmt, std::forward<Args>(args)...);
		}

		template<typename... Args>
		inline void SWAV_API i(fmt::format_string<Args...> fmt, Args&&... args) {
			Logger::get()->info(fmt, std::forward<Args>(args)...);
		}

		template<typename... Args>
		inline void SWAV_API w(fmt::format_string<Args...> fmt, Args&&... args) {
			Logger::get()->warn(fmt, std::forward<Args>(args)...);
		}

		template<typename... Args>
		inline void SWAV_API e(fmt::format_string<Args...> fmt, Args&&... args) {
			Logger::get()->error(fmt, std::forward<Args>(args)...);
		}

		template<typename... Args>
		inline void SWAV_API c(fmt::format_string<Args...> fmt, Args&&... args) {
			Logger::get()->critical(fmt, std::forward<Args>(args)...);
		}

		template<typename... Args>
		inline void SWAV_API t(fmt::format_string<Args...> fmt, Args&&... args) {
