...
std::shared_ptr<swav::Input> getInput(INPUT_MODE mode, int device) {
	...
	if (mode == INPUT_MODE::LOOPBACK) {
		...
		if (device > -1) {
			...
			id = &(swav::getPlaybackDevices()[device].id);
		}
		...
		if (device > -1) {
			devices = swav::getPlaybackDevices();
			id = &(swav::getCaptureDevices()[device].id);
		}
...
std::shared_ptr<swav::Output> getOutput(OUTPUT_MODE mode, int device) {
	...
	if (mode == OUTPUT_MODE::LOCAL) {
		...
		output = std::make_shared<swav::LocalOutput>(id, 4 * (swav::getGlobalContext().sampleRate/100));
	}
	else {
		std::exit(0);
	}
...
void listDevices() {
	...
	for (int i = 0; i < deviceCnt; i++) {
		...
		if (device.type == swav::DeviceType::PLAYBACK)
			if (device.isDefault)
				std::cout << "\t" << i << ". " << device.name << "\033[32m  -> (default)\033[m\n";
			else
...
void start(INPUT_MODE iMode, std::vector<OUTPUT_MODE> oModes, int iDevice, int oDevice) {
...
int setup(int argc, char** argv) {
	...
	std::map<std::string, INPUT_MODE> inputMap{
		{"loopback", INPUT_MODE::LOOPBACK},
	...
	std::map<std::string, OUTPUT_MODE> outputMap{
	...
	app.add_option("--device", outputDevice, "Output device id (required if --outputs includes 'local')");
	...
	app.callback([&]() {
		...
		if (std::find(outputModes.begin(), outputModes.end(), OUTPUT_MODE::LOCAL) != outputModes.end() && outputDevice < 0) {
			throw CLI::ValidationError("You must provide --device when using 'local' output.");
		}
		});
...
int main(int argc, char** argv) {
	...
	start(INPUT_MODE::LOOPBACK, { OUTPUT_MODE::LOCAL }, -1, 1);
}